...
def getPossibleValues(inputString):
    ...
    possibilities = []  # match the length with the full range of values possible for that number of digits.
    ...
    maxValue = 10**total  # e.g. 10**6 = 1,000,000 which is the number of values for '000-000' to '999-999'
    fullRangeInt = range(0,maxValue) # list of ints e.g. 0 to 999,999. No leading zeroes are used.
...
def CalculateProbability (testValue):
    ...
    for index in range(len(possibleValues)):
        possibleValuesNet.append(subTractTerms(possibleValues[index])) # '100-800' becomes -9
    ...
    for items in possibleValuesNet:
        if items in uniquePossibleValues: # for result -9, check if it exists and add 1 to count, otherwise set count to 1
    ...
    print 'Probability of observing net sum: (%i occurences / %i outcomes) = %f %%' % (NumWaysResultCanOccur, TotalAllOutcomes, Probability) 
    print ''
