...
def FacebookPostsURL_user (userID,startDate,endDate,FBtoken):   
    ...
    graphURL =  'https://graph.facebook.com/v2.6/%s/posts?until=%s&limit=100&access_token=%s' % (userID, until, FBtoken)
    return graphURL

def FacebookPostsURL_page (pageName,startDate,endDate,FBtoken):   #start date
    fields = 'created_time,message,id,from,type'
    until = str(endDate + datetime.timedelta(days = 1))
    graphURL =  'https://graph.facebook.com/v2.6/%s/posts?fields=%s&until=%s&limit=100&access_token=%s' % (pageName, fields, until, FBtoken)
    return graphURL
      
def FacebookCountryFollowersURL(pageName,FBtoken):
    graphURL = 'https://graph.facebook.com/v2.6/%s/insights/page_fans_country?until=today&access_token=%s' %(pageName,FBtoken)    
    return graphURL
   
def FacebookGlobalFollowersURL(pageName,FBtoken):
    graphURL = 'https://graph.facebook.com/v2.6/%s?fields=likes&access_token=%s' %(pageName,FBtoken)  
    return graphURL
    
def FacebookPageInsightsURL(pageName, FBtoken):
    graphURL = 'https://graph.facebook.com/v2.6/%s/insights/page_posts_impressions/days_28?access_token=%s' %(pageName,FBtoken)  
    return graphURL
...
def GetZAFollowers(pageName,FBtoken):
    ...
    try:
        ...
        fans = FB_data[0]['values'][length - 1]['value'] # get most recent followers, last on the list        
        fans = fans ['ZA']
    except:
        fans = -1        
    return fans
