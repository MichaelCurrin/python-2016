...
def sendMail(inputText, inputSubject, inputToAddress):
    ...
    text = msg.as_string()
    server.sendmail(
        fromaddr, toaddr, text
    )  # customising the fromaddr as 'python' doesn't affect mail
    server.quit()
...
def TestActiveToken(pageName, FBtoken):
    ...
    graphURL = "https://graph.facebook.com/v2.5/%s?access_token=%s" % (
        pageName,
        FBtoken,
    )
    ...
    try:
        ...
        return (
            "Error",
            "Warning - token not valid! Please extend your token and update it in the script.",
        )
...
def TestFBUserAbout(pageName, FBtoken):
    ...
    """
    if (
        pageName == "n/a" or pageName == "" or pageName == '""'
    ):  # return "OK" if the handle is n/a is empty
        result = "OK"
    else:
        try:
            graphURL = "https://graph.facebook.com/v2.7/%s?access_token=%s" % (
                pageName,
                FBtoken,
            )
            ...
            (
                FB_name,
                FB_id,
            ) = parsed_json.values()  # save results as FB page name and FB page id
...
def FacebookCheck():
    ...
    TokenStatus, TokenTestMessage = TestActiveToken(
        "MichaelCurrinPhotography", ManualExtendedToken
    )
    ...
    if TokenStatus == "OK":
        for clientName in FBInputCompetitorHandles:  # look through client names
            ...
            for index in range(
                len(FBInputCompetitorHandles[clientName])
            ):  # look through handles in client
                str2 = (
                    "    /%s" % FBInputCompetitorHandles[clientName][index]
                )  # e.g. /CastleLite
                str3 = TestFBUserAbout(
                    FBInputCompetitorHandles[clientName][index], ManualExtendedToken
                )  # e.g. OK or Error
    ...
    FBMessage = "\n".join(
        FBMessageList
    )  # combine all FB message list values into single string with line breaks
...
def TwitterReturnUserExists(handle):
    ...
    """
    if (
        handle == "n/a" or handle == "" or handle == '""'
    ):  # return "OK" if the handle is n/a is empty
        result = "OK"
    else:
        try:
...
def TwitterCheck(handle):
    ...
    if TokenStatus == "OK":
        for clientName in TWInputCompetitorHandles:  # look through client names
            ...
            for index in range(
                len(TWInputCompetitorHandles[clientName])
            ):  # look through handles in client
                str2 = (
                    "    %s" % TWInputCompetitorHandles[clientName][index]
                )  # e.g. @AmstelSA
                str3 = TwitterReturnUserExists(
                    TWInputCompetitorHandles[clientName][index]
                )  # e.g. OK or Error
                str4 = str2 + "    " + str3
                # print (str4)
...
FacebookEmailMessage, FBEmailmissing = (
    "",
    0,
)  # default values which can be overwritten by the next line
(
    FacebookEmailMessage,
    FBEmailmissing,
) = FacebookCheck()  # disable this line to turn FB off and still send a mail

TwitterEmailMessage, TwitterEmailMissing = (
    "",
    0,
)  # default values which can be overwritten by the next line
(
    TwitterEmailMessage,
    TwitterEmailMissing,
) = TwitterCheck()  # disable this line to turn FB off and still send a mail
...
Subject = "CRON Social handles - %s errors on %s" % (
    str(FBEmailmissing + TwitterEmailMissing),
    str(CompletedDate),
)
...
for address in addressList:  # loop through addresses
    try:
        sendMail(
            FullMessage, Subject, address
        )  # send mail and display confirmation of recipient address
