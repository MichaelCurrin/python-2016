...
def authorise(service_account_email, key_file_location):
    ...
    connection = get_service('analytics', 'v3', scope, key_file_location, 
...
def get_sessions_totals(service, profile_id, startDate, endDate, 
                                      segment, metrics = 'ga:sessions'):
    """
...
def get_sessions_by_dimensions(service, profile_id, startDate,
                                             endDate, segment, dimensions,
                                             metrics = 'ga:sessions'):
    """
...
def GetAnalyticsData(client_name, site_data, segment_data, goal_data,
                     inputDimensions, inputMetrics, 
                     service_account_email, key_file_location,
                     startDate, endDate, countOfDays,
                     dimensions_data = {}):
    """
    ...
    dateTable = mikes_toolbox.formatTable(('Start Date', 
                                           'End Date', 
                                           'Days'),       
                                          [(startDate, 
                                            endDate, 
    ...
    reportSummary = 'GA Report for %s (%s - %s)' % (client_name,
        ...
        for segmentNames in sortedSegments:
            ...
            if goal_data[sitenames][segmentNames]:
                ...
                if len(goal_data[sitenames][segmentNames])==1: # single goal
                    segmentGoal = 'ga:goal%sCompletions' % \
            ...
            table1 = mikes_toolbox.formatTable (('Site', 'Segment'), 
            ...
            APIresponse = get_sessions_by_dimensions(service, 
                                         site_id, 
            ...
            headerFormatted = map(lambda x: x['name'][3:], 
                                  APIresponse['columnHeaders'])                    
            ...
            if 'rows' in APIresponse.keys():
                ...
                table2 = mikes_toolbox.formatTable(headerFormatted, rowValues,
                                                   setAlign = 'l')
